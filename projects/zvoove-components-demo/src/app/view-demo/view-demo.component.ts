...
import { DefaultZvViewDataSource } from '@zvoove/components/view/src/view-data-source';
import { of } from 'rxjs';
...
export class ViewDemoComponent {
  ...
  public dataSource = new DefaultZvViewDataSource({
  ...
  importsCode = `
import { ZvView, DefaultZvViewDataSource } from '@zvoove/components/view';
import { ZvErrorMessagePipe } from '@zvoove/components/core';
// ...
imports: [
  ZvView,
  ZvErrorMessagePipe, // optional if you want to handle exceptions (like in our example)
],`;

  usageCodeTs = `
private http = inject(HttpClient);
viewDataSource = new DefaultZvViewDataSource({
...
<zv-view [dataSource]="viewDataSource">
  ...
  @if (viewDataSource.exception() !== null) {
    <div>{{ viewDataSource.exception() | zvErrorMessage }}</div>
  }
</zv-view>`;
}
